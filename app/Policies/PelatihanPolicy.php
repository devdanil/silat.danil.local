...
class PelatihanPolicy
{
    ...
    {
        return ($user->hasRolesID([$pelatihan->status->role_id]) && in_array($status_id, [$pelatihan->status->next_id, $pelatihan->status->prev_id])) || ($user->hasRolesID([1]) && ($status_id == 2 || $status_id == 3 || $status_id == 7));
    }
}
